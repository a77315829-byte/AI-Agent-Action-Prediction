...
from feature_utils_qwen_v14_action8 import (
    ACTION_TO_FAMILY,
    ALL_CLASSES,
    FAMILY_NAMES,
    STRUCTURED_DIM,
        ...
        for start in tqdm(
            ...
            for local_index in range(len(batch_segments)):
                ...
                self.input_ids.append(
                    np.asarray(combined_ids[-MAX_LENGTH:], dtype=np.int32)
                )
                self.segment_ids.append(
                    np.asarray(combined_segment_ids[-MAX_LENGTH:], dtype=np.int8)
                )

        if len(self.input_ids) != len(self.labels):
            raise RuntimeError(
                "Tokenization result count does not match label count."
            )
...
class DistillCollator:
    def __init__(self, pad_token_id: int):
        self.base_collator = SegmentCollator(pad_token_id)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V14 Action8 OOF-Teacher Distilled Qwen. This keeps the V4/V12 "
            "model architecture, but changes token segment construction: "
            "MAX_LENGTH=256, original V4 segment budgets, and recent action text=8."
        )
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v14_action8"))
...
def make_datasets(
    ...
    base_dataset = EncodedSegmentDataset(
        subset_samples,
        subset_labels,
        tokenizer,
...
def main() -> None:
    ...
    train_dataset = make_datasets(
        ...
        description="Tokenize V14 action8 train",
    )
    ...
    if args.mode == "eval":
        validation_dataset = make_plain_dataset(
            ...
            tokenizer,
            chunk_size=args.tokenize_chunk_size,
            description="Tokenize V14 action8 validation",
        )
    ...
    print("Load V14 action8 student initialized from V4...")
    ...
    if validation_loader is not None:
        ...
        print(f"Initial V4 with V14-action8 input Macro-F1: {initial_eval['macro_f1']:.6f}")

    metadata = {
        "architecture": "qwen_distill_v14_action8",
        ...
        "segment_config": {
            ...
            "recent_user_text_count": 4,
            "recent_action_text_count": 8,
        },
        "loss_weights": {
    ...
    print(
        f"Start V14 action8 distillation: epochs={args.epochs}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        batch_count = 0
        progress = tqdm(train_loader, desc=f"V14 action8 epoch {epoch}/{args.epochs}")
    ...
    if validation_loader is not None:
        ...
        print(f"Best V14 action8 Macro-F1: {best_eval['macro_f1']:.6f}")
