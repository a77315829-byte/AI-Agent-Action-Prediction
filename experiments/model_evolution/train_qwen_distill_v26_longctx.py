...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V26 long-context OOF-Teacher Distilled Qwen. This keeps the "
            "V4/V12 submission architecture but re-tokenizes samples with a "
            "larger segment budget so Qwen can see more history/action/current text."
        )
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v26_longctx_eval"))
...
def save_model(
    ...
    if evaluation is not None:
        np.savez_compressed(
            output_dir / "validation_logits_v26.npz",
...
def main() -> None:
    ...
    train_dataset = make_datasets(
        ...
        description="Tokenize V26 longctx train",
    ...
    if args.mode == "eval":
        validation_dataset = make_plain_dataset(
            ...
            tokenizer,
            chunk_size=args.tokenize_chunk_size,
            description="Tokenize V26 longctx validation",
            max_length=args.max_length,
            segment_budgets=segment_budgets,
        )
    ...
    print("Load V26 long-context student initialized from:", args.init_dir)
    ...
    metadata = {
        "architecture": "qwen_distill_v26_longctx",
    ...
    print(
        f"Start V26 long-context distillation: epochs={args.epochs}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        batch_count = 0
        progress = tqdm(train_loader, desc=f"V26 epoch {epoch}/{args.epochs}")
    ...
    if validation_loader is not None:
        ...
        print(f"Best V26 longctx Macro-F1: {best_eval['macro_f1']:.6f}")
