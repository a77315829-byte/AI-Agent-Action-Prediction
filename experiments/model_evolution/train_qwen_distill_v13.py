...
import train_qwen_segment_v13 as v4
from feature_utils_qwen_v13 import (
    ACTION_TO_FAMILY,
    ALL_CLASSES,
    FAMILY_NAMES,
    STRUCTURED_DIM,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v13"))
...
def load_model_from_v4(
    ...
    raw_state = safe_torch_load(v4_dir / "heads.pt")
    ...
    for key, value in raw_state.items():
        ...
        if (
            key in ("structured_mlp.0.weight", "structured_mlp.0.bias")
            and getattr(value, "ndim", 0) == 1
            and getattr(target, "ndim", 0) == 1
            and target.shape[0] >= value.shape[0]
        ):
            expanded_value = target.detach().cpu().clone()
            if key.endswith(".weight"):
                expanded_value.fill_(1.0)
            else:
                expanded_value.zero_()
            expanded_value[: value.shape[0]] = value.detach().cpu()
            compatible_state[key] = expanded_value
            expanded.append(
        ...
        if tuple(target.shape) == tuple(value.shape):
            ...
            expanded_value = target.detach().cpu().clone()
            expanded_value.zero_()
            expanded_value[:, : value.shape[1]] = value.detach().cpu()
            compatible_state[key] = expanded_value
            expanded.append(f"{key}: {tuple(value.shape)} -> {tuple(target.shape)}")
        else:
    ...
    if unexpected or missing_non_backbone:
        raise RuntimeError(
            f"Bad V13 head load. missing_non_backbone={missing_non_backbone}, "
            f"unexpected={unexpected}, skipped={skipped[:20]}"
        )
    print(
        f"Loaded compatible V4 heads: {len(compatible_state)} keys. "
        f"Expanded: {expanded}. Skipped mismatched: {len(skipped)}. "
        f"Ignored backbone missing keys: "
