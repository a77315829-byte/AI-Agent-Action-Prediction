...
from feature_utils_qwen_v13_segment import (
    ACTION_TO_FAMILY,
    ALL_CLASSES,
    FAMILY_NAMES,
    STRUCTURED_DIM,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V13 Segment OOF-Teacher Distilled Qwen. This keeps the V4/V12 "
            "model architecture, but changes token segment construction: "
            "MAX_LENGTH=384, action budget=112, and recent action text=8."
        )
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v13_segment"))
...
def main() -> None:
    ...
    if validation_loader is not None:
        ...
        print(f"Initial V4 with V13-segment input Macro-F1: {initial_eval['macro_f1']:.6f}")

    metadata = {
        "architecture": "qwen_distill_v13_segment",
    ...
    print(
        f"Start V13 segment distillation: epochs={args.epochs}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        batch_count = 0
        progress = tqdm(train_loader, desc=f"V13 segment epoch {epoch}/{args.epochs}")
    ...
    if validation_loader is not None:
        ...
        print(f"Best V13 segment Macro-F1: {best_eval['macro_f1']:.6f}")
