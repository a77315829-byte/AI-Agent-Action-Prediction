...
from feature_utils_qwen_v13_clean import (
    ACTION_TO_FAMILY,
    ALL_CLASSES,
    FAMILY_NAMES,
    STRUCTURED_DIM,
    EXTRA_STRUCTURED_DIM,
    build_segments,
    build_structured_features,
    build_extra_structured_features,
)
...
class QwenSegmentClassifier(nn.Module):
    def __init__(
        ...
        # Original V4/V12 branch: keep 95-dim structured input unchanged.
        ...
        # V13 clean branch: additional handcrafted state features are isolated
        # behind zero-initialized residual heads. Therefore, before training,
        # logits are exactly the same as the loaded V4/V12 head logits.
        self.extra_mlp = nn.Sequential(
            nn.LayerNorm(extra_dim),
            nn.Linear(extra_dim, 192),
            nn.GELU(),
            nn.Dropout(0.10),
            nn.Linear(192, 128),
            nn.GELU(),
        )
        self.extra_action_head = nn.Linear(128, num_labels)
        self.extra_family_head = nn.Linear(128, num_families)
        nn.init.zeros_(self.extra_action_head.weight)
        nn.init.zeros_(self.extra_action_head.bias)
        nn.init.zeros_(self.extra_family_head.weight)
        nn.init.zeros_(self.extra_family_head.bias)
    ...
    def forward(
        ...
        base_action_logits = self.action_head(representation)
        base_family_logits = self.family_head(representation)

        extra_representation = self.extra_mlp(extra_features.float())
        action_logits = base_action_logits + self.extra_action_head(extra_representation)
        family_logits = base_family_logits + self.extra_family_head(extra_representation)
        return action_logits, family_logits


class EncodedV13SegmentDataset(Dataset):
    def __init__(
        self,
        ...
        self.extra_features: List[np.ndarray] = []
        ...
        self.extra_features = [
            build_extra_structured_features(sample)
            for sample in samples
        ]
        ...
        for start_index in tqdm(
            ...
            batch_segments = segments[start_index:start_index + chunk_size]
            ...
            for local_index in range(len(batch_segments)):
                ...
                for segment_name in ("history", "action", "meta", "current"):
                    tokens = v4.smart_trim(
                        ...
                        v4.SEGMENT_BUDGETS[segment_name],
                    )
                    combined_ids.extend(tokens)
                    combined_segment_ids.extend(
                        [v4.SEGMENT_IDS[segment_name]] * len(tokens)
                    )
                ...
                self.input_ids.append(
                    np.asarray(combined_ids[-v4.MAX_LENGTH:], dtype=np.int32)
                )
                self.segment_ids.append(
                    np.asarray(combined_segment_ids[-v4.MAX_LENGTH:], dtype=np.int8)
                )
...
class V13SegmentCollator:
    ...
    def __call__(self, features: List[dict]) -> dict:
        ...
        extra_features = torch.tensor(
            np.stack([feature["extra_features"] for feature in features]),
            dtype=torch.float32,
        )
        labels = torch.tensor(
        ...
        family_labels = torch.tensor(
            [feature["family_labels"] for feature in features],
            dtype=torch.long,
        )
...
class DistillCollator:
    def __init__(self, pad_token_id: int):
        self.base_collator = V13SegmentCollator(pad_token_id)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V13 clean OOF-Teacher Distilled Qwen. This keeps the V4 submission "
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v13_clean"))
...
def load_model_from_v4(
    ...
    model = QwenSegmentClassifier(
        ...
        extra_dim=EXTRA_STRUCTURED_DIM,
...
def save_model(
    ...
    if evaluation is not None:
        np.savez_compressed(
            output_dir / "validation_logits_v13_clean.npz",
...
def make_datasets(
    ...
    base_dataset = EncodedV13SegmentDataset(
        subset_samples,
        subset_labels,
        tokenizer,
...
def make_plain_dataset(
    ...
    subset_samples = [samples[int(i)] for i in indices]
    subset_labels = labels[indices]
    return EncodedV13SegmentDataset(
        subset_samples,
        subset_labels,
        tokenizer,
        chunk_size=chunk_size,
...
def main() -> None:
    ...
    train_dataset = make_datasets(
        ...
        description="Tokenize V13 clean train",
    )
    ...
    if args.mode == "eval":
        validation_dataset = make_plain_dataset(
            ...
            tokenizer,
            chunk_size=args.tokenize_chunk_size,
            description="Tokenize V13 clean validation",
        )
        validation_loader = DataLoader(
            validation_dataset,
            batch_size=max(args.validation_batch_size, args.batch_size * 2),
            shuffle=False,
            collate_fn=V13SegmentCollator(tokenizer.pad_token_id),
            num_workers=args.num_workers,
            pin_memory=True,
        )

    print("Load V13 clean student initialized from V4...")
    ...
    metadata = {
        "architecture": "qwen_distill_v13_clean",
        "base_model": model_name,
        "initialized_from": str(args.v4_dir),
        "classes": ALL_CLASSES,
        "families": FAMILY_NAMES,
        "structured_dim": STRUCTURED_DIM,
        "extra_structured_dim": EXTRA_STRUCTURED_DIM,
        "mode": args.mode,
    ...
    print(
        f"Start V13 clean distillation: epochs={args.epochs}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        batch_count = 0
        progress = tqdm(train_loader, desc=f"V13 clean epoch {epoch}/{args.epochs}")
    ...
    if validation_loader is not None:
        ...
        print(f"Best V13 clean Macro-F1: {best_eval['macro_f1']:.6f}")
