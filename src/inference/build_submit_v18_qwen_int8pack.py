...
REQUIREMENTS_TEXT = (
    "# Use the competition evaluation server's preinstalled packages.\n"
)
...
def validate_stage(stage_dir: Path) -> None:
    ...
    required_files = [
        ...
        stage_dir / "model" / "calibrator.joblib",
        stage_dir / "model" / "qwen_config" / "config.json",
        stage_dir / "model" / "qwen_int8" / "index.json",
    ]
    missing = [str(path.relative_to(stage_dir)) for path in required_files if not path.exists()]
    if missing:
        raise RuntimeError("Missing packaged files:\n" + "\n".join(missing))
...
def main() -> None:
    ...
    parser.add_argument("--calibrator-dir", type=Path, default=Path("model/qwen_oof_calibrator_server"))
    parser.add_argument("--script", type=Path, default=Path("script_submission_v18_qwen_int8pack.py"))
    parser.add_argument("--feature-utils", type=Path, default=Path("feature_utils_qwen_v4.py"))
    parser.add_argument("--stage-dir", type=Path, default=Path("submit_stage_v18_qwen_int8pack"))
    parser.add_argument("--zip-path", type=Path, default=Path("submit_v18_qwen_int8pack.zip"))
    ...
    calibrator_dir = resolve(project_dir, args.calibrator_dir)
    ...
    required_sources = [
        ...
        calibrator_dir / "calibrator.joblib",
        inference_script,
        feature_utils,
    ]
    ...
    shutil.copy2(calibrator_dir / "calibrator.joblib", model_dir / "calibrator.joblib")
    ...
    print("V18 int8-pack structure and size checks passed.")
