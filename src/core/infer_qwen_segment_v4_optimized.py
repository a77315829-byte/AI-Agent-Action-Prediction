...
MODEL_DIR = BASE_DIR / "model" / "qwen_segment_v4"
...
def main():
    ...
    postprocess = json.loads(
        (MODEL_DIR / "postprocess.json").read_text(encoding="utf-8")
    )
    ...
    training_class_weights = torch.tensor(
        postprocess["training_class_weights"],
        dtype=torch.float32,
        device="cuda",
    )
    log_training_weights = torch.log(
        training_class_weights.clamp(min=1e-12)
    )
            ...
            with torch.autocast(
                ...
                final_logits = (
                    ...
                    * family_logits[:, family_index]
                    - prior_beta
                    * log_training_weights.unsqueeze(0)
                )
    ...
    for row in rows:
        row["action"] = prediction_map[row["id"]]
