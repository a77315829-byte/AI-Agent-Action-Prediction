...
def resolve_segment_budgets(args: argparse.Namespace) -> Dict[str, int]:
    explicit = {
        "history": int(args.history_budget),
        "action": int(args.action_budget),
        "meta": int(args.meta_budget),
        "current": int(args.current_budget),
    }
    if any(value > 0 for value in explicit.values()):
        missing = [key for key, value in explicit.items() if value <= 0]
        if missing:
            raise ValueError(
                "If any segment budget is supplied, all four budgets must be positive. "
                f"Missing/invalid: {missing}"
            )
        total = sum(explicit.values())
        if total > args.max_length:
            raise ValueError(
                f"Segment budget total {total} exceeds max_length={args.max_length}."
            )
        return explicit

    # Preserve V4's relative allocation 56/72/32/96 while scaling to the
    # requested max_length. The remainder goes to current prompt because the
    # current user instruction is usually the highest-signal segment.
    base = dict(v4.SEGMENT_BUDGETS)
    base_total = sum(base.values())
    budgets = {
        key: max(8, int(round(args.max_length * value / base_total)))
        for key, value in base.items()
    }
    diff = int(args.max_length) - sum(budgets.values())
    budgets["current"] += diff
    if budgets["current"] <= 0:
        raise ValueError(f"Invalid resolved segment budgets: {budgets}")
    return budgets


class LongEncodedSegmentDataset(Dataset):
    def __init__(
        self,
        ...
        for start in tqdm(range(0, len(samples), chunk_size), desc=description):
            ...
            for local_index in range(len(batch_segments)):
                ...
                for segment_name in ("history", "action", "meta", "current"):
                    tokens = v4.smart_trim(
                        encoded_by_segment[segment_name][local_index],
                        self.segment_budgets[segment_name],
                    )
                    combined_ids.extend(tokens)
                    combined_segment_ids.extend([v4.SEGMENT_IDS[segment_name]] * len(tokens))

                if not combined_ids:
                    fallback = tokenizer.eos_token_id if tokenizer.eos_token_id is not None else tokenizer.pad_token_id
                    combined_ids = [fallback]
                    combined_segment_ids = [v4.SEGMENT_IDS["current"]]

                self.input_ids.append(np.asarray(combined_ids[-self.max_length:], dtype=np.int32))
                self.segment_ids.append(np.asarray(combined_segment_ids[-self.max_length:], dtype=np.int8))
...
class DistillDataset(Dataset):
    def __init__(
        ...
        self.tree_probabilities = tree_probabilities.astype(np.float32)
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        item["tree_probabilities"] = torch.tensor(
            self.tree_probabilities[index],
            dtype=torch.float32,
        )
        return item
...
class DistillCollator:
    def __init__(self, pad_token_id: int):
        self.base_collator = LongSegmentCollator(pad_token_id)
    ...
    def __call__(self, features: List[dict]) -> dict:
        ...
        tree_probabilities = torch.stack([f.pop("tree_probabilities") for f in features])
        ...
        batch["tree_probabilities"] = tree_probabilities
        return batch
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V27 Tree-Teacher Distilled Qwen. This keeps the V4/V12 "
            "submission architecture and adds a low-weight V23 LightGBM "
            "tree-probability teacher to the distillation objective."
        )
    )
    ...
    parser.add_argument("--init-dir", "--v4-dir", dest="init_dir", type=Path, default=Path("model/qwen_distill_v12_eval"),
                        help="Checkpoint directory containing adapter/, heads.pt, metadata.json, postprocess.json. "
                             "Use model/qwen_segment_v4 for clean V4 init or qwen_distill_v12_eval for fast continuation.")
    parser.add_argument("--oof-logits", type=Path, default=Path("model/qwen_v4_oof/oof_logits_all_70000.npz"))
    parser.add_argument("--tree-prob", type=Path, default=Path("model/v27_tree_teacher/tree_prob_all.npy"),
                        help="Numpy array of shape [num_train_rows, num_classes] with V23 tree probabilities.")
    parser.add_argument("--output-dir", type=Path, default=Path("model/qwen_distill_v27_tree_teacher_eval"))
    ...
    parser.add_argument("--tree-distill-weight", type=float, default=0.05)
    parser.add_argument("--tree-temperature", type=float, default=1.0)
    parser.add_argument("--tree-smoothing", type=float, default=0.02)
    parser.add_argument("--tree-gate-mode", choices=["none", "complementarity", "complementarity_confidence"],
                         default="complementarity",
                         help="none = original ungated behavior (tree KL applied to every row); "
                              "complementarity = only apply where tree_pred==true_label AND qwen_teacher_pred!=true_label "
                              "(the rows where tree actually corrects Qwen); "
                              "complementarity_confidence = complementarity AND tree confidence >= --tree-confidence-threshold")
    parser.add_argument("--tree-confidence-threshold", type=float, default=0.6)
    parser.add_argument(
        "--qwen-distill-scale-on-tree-gate",
        type=float,
        default=0.25,
        help=(
            "Scale Qwen final/action distillation and teacher-CE on rows where the tree "
            "complementarity gate fires. 0 disables the wrong Qwen action teacher on those rows; "
            "1 preserves the original behavior. Family distillation is intentionally left unchanged."
        ),
    )
...
def load_model_from_init(
    ...
    result = model.load_state_dict(
        safe_torch_load(init_dir / "heads.pt"),
        strict=False,
    )
    ...
    if unexpected or missing_non_backbone:
        raise RuntimeError(
            f"Bad init head load. missing_non_backbone={missing_non_backbone}, "
            f"unexpected={unexpected}"
        )
    print(
        f"Loaded init heads. Ignored backbone missing keys: "
...
def build_tree_gate_mask(
    tree_probabilities: torch.Tensor,
    labels: torch.Tensor,
    qwen_teacher_logits: torch.Tensor,
    gate_mode: str,
    ...
    tree_pred = tree_probabilities.argmax(dim=1)
    qwen_pred = qwen_teacher_logits.argmax(dim=1)
    mask = (tree_pred == labels) & (qwen_pred != labels)

    if gate_mode == "complementarity_confidence":
        tree_confidence = tree_probabilities.max(dim=1).values
        mask = mask & (tree_confidence >= confidence_threshold)

    return mask


def gated_tree_distill_loss(
    student_logits: torch.Tensor,
    tree_probabilities: torch.Tensor,
    ...
    """Compute tree KL only on selected rows.

    The loss is normalized over selected rows, while batches with no selected
    rows contribute exactly zero. Qwen teacher losses are separately reduced
    on the same rows in the training loop.
    """
    if not gate_mask.any():
        return torch.zeros((), dtype=student_logits.dtype, device=student_logits.device), 0

    masked_student = student_logits[gate_mask]
    masked_tree = tree_probabilities[gate_mask]
    loss = kl_divergence_from_probabilities(
        masked_student,
        masked_tree,
        temperature,
        smoothing,
    )
    return loss, int(gate_mask.sum().item())
...
def kl_divergence_from_probabilities(
    ...
    teacher = teacher_probabilities.float().clamp(min=1e-8)
    teacher = teacher / teacher.sum(dim=-1, keepdim=True).clamp(min=1e-8)
    if smoothing > 0.0:
        teacher = (1.0 - smoothing) * teacher + smoothing / teacher.shape[-1]
        teacher = teacher / teacher.sum(dim=-1, keepdim=True).clamp(min=1e-8)
    student = student_logits.float() / temperature
    return (
        F.kl_div(
...
def evaluate(
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Validation", leave=False):
            ...
            batch.pop("tree_probabilities", None)
...
def save_model(
    ...
    if evaluation is not None:
        np.savez_compressed(
            output_dir / "validation_logits_v27.npz",
...
def make_datasets(
    ...
    base_dataset = LongEncodedSegmentDataset(
        subset_samples,
        subset_labels,
        tokenizer,
...
def make_plain_dataset(
    ...
    subset_samples = [samples[int(i)] for i in indices]
    subset_labels = labels[indices]
    return LongEncodedSegmentDataset(
        subset_samples,
        subset_labels,
        tokenizer,
        chunk_size=chunk_size,
...
def main() -> None:
    ...
    print("Load tree teacher probabilities:", args.tree_prob)
    tree_probabilities = np.load(args.tree_prob).astype(np.float32)
    if tree_probabilities.shape != (len(labels), NUM_CLASSES):
        raise RuntimeError(
            f"tree_prob shape mismatch: {tree_probabilities.shape} != {(len(labels), NUM_CLASSES)}"
        )
    tree_probabilities = tree_probabilities / np.maximum(tree_probabilities.sum(axis=1, keepdims=True), 1e-8)
    ...
    segment_budgets = resolve_segment_budgets(args)
    ...
    if args.smoke:
        ...
        tree_probabilities = tree_probabilities[selected]
        args.epochs = 1
    ...
    train_dataset = make_datasets(
        ...
        description="Tokenize V27 tree-teacher train",
    ...
    if args.mode == "eval":
        validation_dataset = make_plain_dataset(
            ...
            tokenizer,
            chunk_size=args.tokenize_chunk_size,
            description="Tokenize V27 tree-teacher validation",
            max_length=args.max_length,
            segment_budgets=segment_budgets,
        )
        validation_loader = DataLoader(
            ...
            shuffle=False,
            collate_fn=LongSegmentCollator(tokenizer.pad_token_id),
            num_workers=args.num_workers,
            pin_memory=True,
        )

    print("Load V27 tree-teacher student initialized from:", args.init_dir)
    model = load_model_from_init(
        args.init_dir,
        model_name,
        tokenizer,
        device,
    ...
    metadata = {
        "architecture": "qwen_distill_v27_tree_teacher",
        "base_model": model_name,
        "initialized_from": str(args.init_dir),
        ...
        "loss_weights": {
            ...
            "tree_distill": float(args.tree_distill_weight),
            "tree_temperature": float(args.tree_temperature),
            "tree_smoothing": float(args.tree_smoothing),
            "tree_gate_mode": args.tree_gate_mode,
            "tree_confidence_threshold": float(args.tree_confidence_threshold),
            "qwen_distill_scale_on_tree_gate": float(args.qwen_distill_scale_on_tree_gate),
        },
        "learning_rates": {
    ...
    print(
        f"Start V27 tree-teacher distillation: epochs={args.epochs}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tree_gate_fired_total = 0
        tree_gate_fired_batches = 0
        tree_gate_batches = 0
        tree_gate_rows_seen = 0
        batch_count = 0
        progress = tqdm(train_loader, desc=f"V27 epoch {epoch}/{args.epochs}")
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            tree_prob_batch = batch.pop("tree_probabilities").to(device, non_blocking=True)
            batch = {key: value.to(device, non_blocking=True) for key, value in batch.items()}
            ...
            tree_gate_mask = build_tree_gate_mask(
                tree_prob_batch,
                labels_tensor,
                teacher_final,
                args.tree_gate_mode,
                args.tree_confidence_threshold,
            )

            # The Qwen OOF teacher is wrong by construction on complementarity
            # rows. Reduce its action-level pressure there instead of allowing
            # a 0.05 tree term to fight the much larger Qwen KD terms unchanged.
            qwen_distill_scale = torch.ones(
                labels_tensor.shape[0],
                dtype=torch.float32,
                device=device,
            )
            if args.tree_gate_mode != "none":
                qwen_distill_scale[tree_gate_mask] = float(args.qwen_distill_scale_on_tree_gate)

            distill_final_per_row = kl_divergence_per_sample(
                student_final,
                teacher_final,
                args.distill_temperature,
            )
            distill_final_loss = (distill_final_per_row * qwen_distill_scale).mean()

            distill_action_per_row = kl_divergence_per_sample(
                action_logits,
                teacher_action,
                args.distill_temperature,
            )
            distill_action_loss = (distill_action_per_row * qwen_distill_scale).mean()

            # Family supervision can still be correct when the action top-1 is
            # wrong, so keep family KD unchanged.
            distill_family_loss = kl_divergence(
                family_logits,
                teacher_family,
                args.distill_temperature,
            )
            tree_distill_loss, tree_gate_count = gated_tree_distill_loss(
                student_final,
                tree_prob_batch,
                tree_gate_mask,
                args.tree_temperature,
                args.tree_smoothing,
            )
            ...
            raw_loss = (
                ...
                + args.tree_distill_weight * tree_distill_loss
            )
            ...
            running["tree_distill"] += float(tree_distill_loss.detach().item())
            tree_gate_fired_total += tree_gate_count
            tree_gate_fired_batches += int(tree_gate_count > 0)
            tree_gate_batches += 1
            tree_gate_rows_seen += int(len(labels_tensor))
            batch_count += 1
            ...
            if step % 100 == 0:
                progress.set_postfix(
                    ...
                    tree=f"{tree_distill_loss.detach().item():.3f}",
                    tree_gate=f"{tree_gate_count}/{len(labels_tensor)}",
        ...
        record = {
            ...
            "tree_gate_mode": args.tree_gate_mode,
            "tree_gate_fired_total": int(tree_gate_fired_total),
            "tree_gate_fired_batches": int(tree_gate_fired_batches),
            "tree_gate_batches": int(tree_gate_batches),
            "tree_gate_rows_seen": int(tree_gate_rows_seen),
            "tree_gate_rate": float(tree_gate_fired_total / max(1, tree_gate_rows_seen)), 
        }
        print(
            f"  tree gate fired on {tree_gate_fired_total}/{tree_gate_rows_seen} rows "
            f"({100.0 * tree_gate_fired_total / max(1, tree_gate_rows_seen):.3f}%), "
            f"in {tree_gate_fired_batches}/{tree_gate_batches} batches this epoch"
        )
    ...
    if validation_loader is not None:
        ...
        print(f"Best V27 tree-teacher Macro-F1: {best_eval['macro_f1']:.6f}")
        ...
        metadata["validation_macro_f1"] = float(best_eval["macro_f1"])
